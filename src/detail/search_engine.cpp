...
int SearchEngine::lowerBound(const uint8_t* key, int len, MBData& data, std::string* bound_key)
{
    ...
    auto do_bound = [&](size_t root_off) -> int {
        ...
        if (edge_len > LOCAL_EDGE_LEN) {
            ...
            if (dict.mm.ReadData(bound_state.node_buff, edge_len_m1, edge_str_off) != edge_len_m1)
                return MBError::READ_ERROR;
            key_buff = bound_state.node_buff;
        } else {
        ...
        if (edge_len < len) {
            ...
            rval = traverseToLowerBound(p, len, edge_ptrs, data, bound_edge_ptrs, bound_state);
...
int SearchEngine::upperBound(const uint8_t* key, int len, MBData& data, std::string* bound_key)
{
    // Placeholder; implement analogous to lowerBound when upper bound traversal is exposed.
    (void)key; (void)len; (void)data; (void)bound_key;
    ...
    MBData& data, EdgePtrs& bound_edge_ptrs, BoundSearchState& state) const
{
    ...
    while (true) {
        int new_le_key = -1;
        int rval = dict.mm.NextLowerBoundEdge(key, len, edge_ptrs, state.node_buff, data, bound_edge_ptrs, new_le_key);

        if (state.bound_key && new_le_key >= 0) {
            state.le_match_len = data.match_len;
            state.le_edge_key = new_le_key;
        }
        ...
        if (edge_len > LOCAL_EDGE_LEN) {
            size_t edge_str_off = Get5BInteger(edge_ptrs.ptr);
            if (dict.mm.ReadData(state.node_buff, edge_len_m1, edge_str_off) != edge_len_m1)
                return MBError::READ_ERROR;
            key_buff = state.node_buff;
        } else {
            key_buff = edge_ptrs.ptr;
        }

        if (edge_len_m1 > 0) {
            int cmp = memcmp(key_buff, key + 1, edge_len_m1);
            if (cmp != 0) {
                if (cmp < 0) {
                    state.use_curr_edge = true;
                    if (state.bound_key) {
                        state.bound_key->append(reinterpret_cast<const char*>(state.key), data.match_len + 1);
                        state.bound_key->append(reinterpret_cast<const char*>(key_buff), edge_len_m1);
                    }
                }
                return MBError::NOT_EXIST;
            }
